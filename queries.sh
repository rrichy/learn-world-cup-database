...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams LEFT JOIN games AS wg ON teams.team_id = wg.winner_id LEFT JOIN games AS og ON teams.team_id = og.opponent_id WHERE (wg.year = 2014 AND wg.round = 'Eighth-Final') OR (og.year = 2014 AND og.round = 'Eighth-Final')")"
